...
namespace GeographicLib {
  ...
  private:
    // s_ + t_ accumulators for the sum.
    T s_, t_;
    ...
    void Add(T y) {
      ...
      y  = Math::sum(y, t_,  u);
      s_ = Math::sum(y, s_, t_);
      // Start is s_, t_ decreasing and non-adjacent.  Sum is now (s + t + u)
      ...
      // Ideally, we want s_ = round(s+t+u) and _u = round(s+t+u - s_).  The
      ...
      //
      // Case 1: s_ is not equal to round(s+t+u) -- off by 1 ulp
      // [12, -1] - 8 -> [4, 0, -1] -> [4, -1] = 3 should be [3, 0] = 3
      //
      // Case 2: s_+t_ is not as close to s+t+u as it shold be
      ...
      // canonical, because s_ = round(s_ + t_) may not be true.  For example,
      // with 3-bit floats:
      //
      // [128, 16] + 1 -> [160, -16] -- 160 = round(145).
      // But [160, 0] - 16 -> [128, 16] -- 128 = round(144).
      //
      if (s_ == 0)              // This implies t == 0,
        s_ = u;                 // so result is u
      else
        t_ += u;                // otherwise just accumulate u to t.
    }
    T Sum(T y) const {
      Accumulator a(*this);
      a.Add(y);
  ...
  public:
     ...
     * @param[in] y set \e sum = \e y.
     **********************************************************************/
    Accumulator& operator=(T y) { s_ = y; t_ = 0; return *this; }
    /**
     ...
     **********************************************************************/
    Accumulator& operator*=(int n) { s_ *= n; t_ *= n; return *this; }
    /**
    ...
    Accumulator& operator*=(T y) {
      using std::fma;
      T d = s_; s_ *= y;
      d = fma(y, d, -s_); // the error in the first multiplication
      t_ = fma(y, t_, d); // add error to the second term
      return *this;
    }
    /**
     ...
     **********************************************************************/
    bool operator==(T y) const { return s_ == y; }
    /**
