...
namespace GeographicLib {
  ...
  real DAuxLatitude::DRectifying(const AuxAngle& phi1,
                                       const AuxAngle& phi2)
    const {
    ...
    if (x == y) {
      ...
      return base::b_ * dEdbet / base::RectifyingRadius(true) * dbetdphi;
    }
  }

  real DAuxLatitude::DParametric(const AuxAngle& phi1,
                                       const AuxAngle& phi2)
    const {
    ...
    if (!(tx * ty >= 0))        // This includes, e.g., tx = 0, ty = inf
      r = (atan(base::fm1_ * ty) - atan(base::fm1_ * tx)) /
        (atan(ty) - atan(tx));
    else if (tx == ty) {        // This includes the case tx = ty = inf
      tx *= tx;
      if (tx <= 1)
        r = base::fm1_ * (1 + tx) / (1 + base::e2m1_ * tx);
      else {
        tx = 1/tx;
        r = base::fm1_ * (1 + tx) / (base::e2m1_ + tx);
      }
    } else {
      if (tx * ty <= 1)
        r = atan2(base::fm1_ * (ty - tx), 1 + base::e2m1_ * tx * ty)
          / atan2(        ty - tx , 1 +         tx * ty);
      else {
        tx = 1/tx; ty = 1/ty;
        r = atan2(base::fm1_ * (ty - tx), base::e2m1_ + tx * ty)
          / atan2(        ty - tx ,   1   + tx * ty);
      }
    }
    return r;
  ...
  real DAuxLatitude::DE(const AuxAngle& X, const AuxAngle& Y) const {
    ...
    return (Ezbsz - k2 * sx * sy) * Dsz / (flip ? 1 - base::f_ : 1);
  }

  /// \cond SKIP
  real DAuxLatitude::Dsn(real x, real y) {
    real sc1 = base::sc(x);
  ...
  real DAuxLatitude::Datan(real x, real y) {
  ...
  real DAuxLatitude::Dasinh(real x, real y) {
    using std::isinf;           // Needed for Centos 7, ubuntu 14
  ...
  real DAuxLatitude::Dh(real x, real y) {
  ...
  real DAuxLatitude::Datanhee(real x, real y) const {
    // atan(e*sn(tphi))/e:
    ...
    return base::f_ < 0 ?
      Datan(base::e_ * base::sn(x), base::e_ * base::sn(y)) * Dsn(x, y) :
      Dasinh(base::e1_ * base::sn(base::fm1_ * x),
             base::e1_ * base::sn(base::fm1_ * y)) *
      Dsn(base::fm1_ * x, base::fm1_ * y);
  }
  /// \endcond

  real DAuxLatitude::DIsometric(const AuxAngle& phi1,
                                      const AuxAngle& phi2)
    const {
    ...
    return isnan(tphi1) || isnan(tphi2) ? numeric_limits<real>::quiet_NaN() :
      (isinf(tphi1) || isinf(tphi2) ? numeric_limits<real>::infinity() :
       (Dasinh(tphi1, tphi2) - base::e2_ * Datanhee(tphi1, tphi2)) /
       Datan(tphi1, tphi2));
  }

  real DAuxLatitude::DConvert(int auxin, int auxout,
    ...
    if ( isnan(base::c_[base::Lmax * (k + 1) - 1]) )
    ...
    return 1 + DClenshaw(true, zeta2n.radians() - zeta1n.radians(),
                         ...
                         base::c_ + base::Lmax * k, base::Lmax);
  }

  real DAuxLatitude::DClenshaw(bool sinp, real Delta,
